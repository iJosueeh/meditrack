...
public class MotivoMovimientoDAO extends JdbcDaoSupport {
    ...
    public void eliminar(String id) throws SQLException {
        String checkSql = "SELECT COUNT(*) FROM movimientos WHERE motivo_id = ?";
        String deleteSql = "DELETE FROM motivos_movimiento WHERE id = ?";
        try (Connection conn = getConnection()) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(checkSql)) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next() && rs.getInt(1) > 0) {
                            conn.rollback();
                            throw new SQLException("No se puede eliminar el motivo de movimiento porque tiene movimientos asociados.");
                        }
                    }
                }
