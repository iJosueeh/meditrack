...
public class JdbcSedeRepository implements SedeRepository {
    ...
    public Sede save(Sede sede) {
        try {
            ...
            String sql = "INSERT INTO sedes (id, nombre, direccion, is_activa, telefono, ubigeo, tipo_sede, capacidad_almacen, administrador_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            try (Connection conn = dbConfig.getConnection();
                 PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public Sede update(Sede sede) {
        String sql = "UPDATE sedes SET nombre = ?, direccion = ?, is_activa = ?, telefono = ?, ubigeo = ?, tipo_sede = ?, capacidad_almacen = ?, administrador_id = ? WHERE id = ?";
        try (Connection conn = dbConfig.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
